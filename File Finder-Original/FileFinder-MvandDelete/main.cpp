...
int extractVids(const fs::path &src, fs::path &dest){
    ...
    for(auto &file: fs::recursive_directory_iterator(src)){
        ...
        if(checker2 == 2){
            ...
            //Now Copy data over
            fs::rename(file, newFName); 
            vidCntR++; //end of if the file is an image
...
int main(int argc, char **argv){
    ...
    do{
        ...
        switch(lowerAns){
            ...
            case 'i':{
                std::cout<<"Processing and Moving, Please Wait..."<<std::endl; 
                ...
                std::cout<<"Moving Images, Please Wait..."<<std::endl; 
                picCnt = extractPics(srcFolder, destFolder); 
                std::cout<<"Image Move Complete"<<std::endl; 
    ...
    std::cout<<"Images Moved = "<<picCnt<<std::endl; 
    std::cout<<"Videos Moved = "<<vidCnt<<std::endl; 
